let Stamp = (function () {
  ...
  Constructor.prototype.fromNow = function (n, units) {
    ...
    if (!times[units]) {
      console.log(
        'units is not a valid time unit string, returning current timestamp',
      );
      return this;
    }
    ...
    this.date = new Date(newTime).toLocaleString(
      this.dateLocale,
      this.dateFormat,
    );
    return this;
...
console.log(
  `In 5 minutes using fromNow, timestamp: ${addSubtractInFiveMin.timestamp}, date: ${addSubtractInFiveMin.date}`,
);
console.log(
  `In 20 seconds using secondsFromNow, timestamp: ${twentySecondsFromNow.timestamp}, date: ${twentySecondsFromNow.date}`,
);
console.log(
  `In 20 minutes using minutesFromNow, timestamp: ${twentyMinutesFromNow.timestamp}, date: ${twentyMinutesFromNow.date}`,
);
console.log(
  `In 3 hours using hoursFromNow, timestamp: ${threeHoursFromNow.timestamp}, date: ${threeHoursFromNow.date}`,
);
console.log(
  `In 3 days using daysFromNow, timestamp: ${threeDaysFromNow.timestamp}, date: ${threeDaysFromNow.date}`,
);
console.log(
  `In 3 weeks using weeksFromNow, timestamp: ${threeWeeksFromNow.timestamp}, date: ${threeWeeksFromNow.date}`,
);
console.log(
  `In 3 months using monthsFromNow, timestamp: ${threeMonthsFromNow.timestamp}, date: ${threeMonthsFromNow.date}`,
);
console.log(
  `In 3 years using yearsFromNow, timestamp: ${threeYearsFromNow.timestamp}, date: ${threeYearsFromNow.date}`,
);

console.log(
  `Stamp object 3 years, 3 months, 3 weeks, 3 hours, 3 minutes, and 3 seconds from now: ${JSON.stringify(
    new Stamp()
      .yearsFromNow(3)
      .monthsFromNow(3)
      .weeksFromNow(3)
...
form.addEventListener('submit', function (event) {
  ...
  results.innerText = `The date and time ${Number(formNum.value)} ${
    formUnits.value
  } from now will be ${formTime.date}`;
});
